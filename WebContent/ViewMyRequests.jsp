...
<title>View my smart working requests</title>
</head>
<body>
...
<div id="global">
    <h3 align="center">Showing smart working requests for user <%=session.getAttribute("name")%> for the current period.</h3>
    <p>
    	...
    	<c:forEach items="${viewmyrequests}" var="userPlanning">
			<tr>
				<td>${userPlanning.userId}</td>
				<td>${userPlanning.requestDate}</td>
				<td>${userPlanning.mondayFirstWeek}</td>
				<td>${userPlanning.tuesdayFirstWeek}</td>
				<td>${userPlanning.wednesdayFirstWeek}</td>
				<td>${userPlanning.thursdayFirstWeek}</td>
				<td>${userPlanning.fridayFirstWeek}</td>
				<td>${userPlanning.mondaySecondWeek}</td>
				<td>${userPlanning.tuesdaySecondWeek}</td>
				<td>${userPlanning.wednesdaySecondWeek}</td>
				<td>${userPlanning.thursdaySecondWeek}</td>
				<td>${userPlanning.fridaySecondWeek}</td>
				<c:choose>
      				<c:when test="${userPlanning.status == 'requested'}">
           				<td style="color:orange;  font-weight:bold;">${userPlanning.status}</td>
      				</c:when>	
	  				<c:when test="${userPlanning.status == 'approved'}">
           				<td style="color:green;  font-weight:bold;">${userPlanning.status}</td>
      				</c:when>
      				<c:when test="${userPlanning.status == 'rejected'}">
           				<td style="color:red;  font-weight:bold;">${userPlanning.status}</td>
      				</c:when>
				</c:choose>
			</tr>
		</c:forEach>
